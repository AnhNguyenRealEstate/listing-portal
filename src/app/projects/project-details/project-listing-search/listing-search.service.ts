...
export class ListingSearchService {
    ...
    async getListingsByCriteria(newSearchCriteria: SearchCriteria, projectId: string): Promise<Listing[]> {
        this.searchInProgress$$.next(true);

        newSearchCriteria = { ...newSearchCriteria };
        const results: Listing[] = [];

        if (this.isNewSnapshotNeeded(newSearchCriteria, projectId)) {
    ...
    private isNewSnapshotNeeded(newSearchCriteria: SearchCriteria, newProjetId: string) {
        return !this.querySnapshot
            || (this.currentProjectId !== newProjetId)
            || (this.currentSearchCriteria.category !== newSearchCriteria.category)
    ...
    private criteriaToQuery(
        ...
        let q = query(
            listings,
            where('purpose', '==', criteria.purpose?.trim() || 'For Rent'),
